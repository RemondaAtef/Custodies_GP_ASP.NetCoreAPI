...
namespace DB3GP.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public IActionResult GetRequestsToResponse(string userid)  //Get All items
        {
            var data = Admin.UpdateDatabase1(userid);

            return Ok(data);
        }

        [HttpPost("Response")]
        public void Responsetorequest(string requestStatus, string userid)
        {
            Admin.UpdateDatabase(requestStatus, userid);
        }

        [HttpPost("ResponseForFurniture")]
        public IActionResult Responsetorequest2(string requestStatus, string userid)
        {
           var data = Admin.UpdateDatabase3(requestStatus, userid);
