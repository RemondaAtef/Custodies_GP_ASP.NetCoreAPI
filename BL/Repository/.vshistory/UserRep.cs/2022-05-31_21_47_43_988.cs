...
namespace DB3GP.BL.Repository
{
    public class UserRep : IUserRep
    {
       ...
       public void RequestToRestore(string userid,int itemid, string statusAfterRestore)
        {
            var data = ShowMyOwnership(userid);
        ...
        public OwnerShip ShowMyOwnership(string userid)   //Show myOwnerShip by userID
        {
            var data = db.OwnerShip.Where(a => a.UserId == userid)
                .Select(a => new OwnerShip { OwnerShipID = a.OwnerShipID,  UserId = a.UserId, ItemId = a.ItemId, OwnershipDate = a.OwnershipDate }).FirstOrDefault();
