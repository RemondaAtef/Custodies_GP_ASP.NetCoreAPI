...
namespace DB3GP.BL.Repository
{
    public class UserRep : IUserRep
    {
        ...
        public Member ViewProfile2(string email)       //when click edit button hy3rd el data bta3te  => Update
        {
        ...
        public ViewDetailsVM ViewDetails(int itemID)   //view item details by itemID
        {
            var data = db.Item.Where(a => a.ItemId == itemID)
                              .Select(a => new ViewDetailsVM() { ItemName = a.ItemName, ItemType = a.ItemType, Quantity = a.Quantity, Serial = a.Serial , Image = a.Image })
                              .FirstOrDefault();
            return data;
        }

        public IEnumerable<ViewCartVM> ViewCart(string userId)  //View My Cart by UserID
        {
            var data = db.Cart.Where(a => a.UserId == userId)
                              .Select(a => new ViewCartVM() { ItemName = a.Item.ItemName , Image = a.Item.Image, Quantity = a.CartQuantity});


            return data;
        }

        public IEnumerable<ItemNamePhoto> Search(string name)   //Search item by itemName
        {
        ...
        public void DeleteFromCart(int itemid, string userid)  //delete item from my cart by itemID & UserID
        {
            Cart cart;

            cart = db.Cart.Where(a => a.ItemId == itemid && a.UserId == userid)
                          .First();
        ...
        public Cart RequestToTake(string userid)     //View my Cart by userID to Request to take Items
        {
            var data = db.Cart.Where(a => a.UserId == userid)
        ...
        public OwnerShip RequestToRestore2(string userid, int itemid)   //show my Ownership by userid , itemid to request to restore item  
        {
            var data = db.OwnerShip.Where(a => a.UserId == userid && a.ItemId == itemid)
                                  .Select(a => new OwnerShip { OwnerShipID = a.OwnerShipID, UserId = a.UserId, ItemId = a.ItemId, OwnershipDate = a.OwnershipDate, OwnerShipQuantity = a.OwnerShipQuantity })
                                  .FirstOrDefault();

            return data;
        }

        public void RequestToRestore(string userid,int itemid, string statusAfterRestore)  //Add item in request table 
        {
        ...
        public IEnumerable<ItemNamePhoto> PopularItems()   //Show Popular items
        {
        ...
        {
            Member m = new Member();
            signin dpt = new signin();

            if (username == "Moheb" || username == "Remondaa")
            {
                var data3 = db.Request.Select(a => new getAllRequestsVM() { ItemID = a.ItemId, RequestStatus = a.RequestStatus, UserID = a.UserId, UserName = a.Member.Name, ItemName = a.Item.ItemName });

                return (signin)data3;
            }
            else if(m.status == MemberStatus.Active)
            {
                var data = db.Member.Where(a => a.Name == username && a.Password == password)
                       .Select(a => new Member { Name = a.Name, Email = a.Email })
                       .FirstOrDefault();

                // Console.WriteLine(username, password);

                dpt.Popular = db.Item.Where(a => a.Popular == "True")
                                      .Select(a => new ItemNamePhoto() { ItemName = a.ItemName, Image = a.Image });
