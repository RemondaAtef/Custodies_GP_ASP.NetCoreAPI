...
namespace DB3GP.BL.Repository
{
    public class UserRep : IUserRep
    {
        ...
        public void DeleteFromCart(int itemid, string userid)  //delete item from my cart by itemID
        {
            Cart cart;
        ...
        public OwnerShip RequestToRestore2(string userid, int itemid)   //search in Ownership by userid , itemid to request to restore item  
        {
        ...
        {
            var data = db.Member.Where(a => a.Name == dpt.UserName && a.Password == dpt.Password)
               .Select(a => new Member { Name = a.Name, Email = a.Email })
               .FirstOrDefault();

            Console.WriteLine(dpt.UserName, dpt.Password);
