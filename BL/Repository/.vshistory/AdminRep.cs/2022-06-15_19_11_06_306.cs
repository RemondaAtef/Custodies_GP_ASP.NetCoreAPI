          ...
          //  Item I = new Item();
            ...
            if (requestStatus == "Accept" )
            {
                ...
                var data1 = db.Item.Where(a => a.ItemType == "Furniture")
                                     .Select(a => new Item { ItemId = a.ItemId, ItemName = a.ItemName, ItemType = a.ItemType, ItemStatus = a.ItemStatus, Image = a.Image, Popular = a.Popular, Serial = a.Serial, UnitPrice = a.UnitPrice, DateUpdated = DateTime.Now, Quantity = a.Quantity - data.RequestQuantity })
                                     .FirstOrDefault();
