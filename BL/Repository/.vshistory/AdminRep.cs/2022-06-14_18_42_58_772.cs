...
namespace DB3GP.BL.Repository
{
    public class AdminRep : IAdminRep
    {
        ...
        public IEnumerable<Request> UpdateDatabase1()
        {
            //var data = db.Request.Where(a => a.TakeOrRestore == "Take")
           // var data = db.Request.Select(a => new Request { UserId = a.UserId, ItemId = a.ItemId, RequestID = a.RequestID, RequestQuantity = a.RequestQuantity, RequestStatus = a.RequestStatus, StatusAfterRestore = a.StatusAfterRestore, TakeOrRestore = a.TakeOrRestore });
               // .FirstOrDefault();
            var data = db.Request;

            return data;
        }
        //public IEnumerable<Request> getAllRequests()    //Get All items
        //{
        //    var data = db.Item;

        //    return data;
        //}
        public void UpdateDatabase(string requestStatus)
        {
            // var data = UpdateDatabase1();

            Request data = new Request();

            if (data.TakeOrRestore == "Take" && requestStatus == "Accept")
            {
