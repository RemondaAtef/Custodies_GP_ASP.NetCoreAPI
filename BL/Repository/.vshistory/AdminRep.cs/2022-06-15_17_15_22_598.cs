...
namespace DB3GP.BL.Repository
{
    public class AdminRep : IAdminRep
    {
        ...
        public OwnerShip UpdateDatabase(string requestStatus, string userid)
        {
            ...
            if (requestStatus == "Accept" && data.RequestQuantity <= I.Quantity)
            {
        ...
        public void UpdateDatabase3(string requestStatus, string userid)
        {
           var d1 =  UpdateDatabase(requestStatus , userid);
                       

            var data = db.Item.Where(a => a.ItemType == "Furniture" || a.ItemType == "Electronic Device")
