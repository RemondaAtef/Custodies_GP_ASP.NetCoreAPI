    ...
    public class AdminRep : IAdminRep
    {
        ...
        public void ReplyForRequest(string requestStatus, string userid ,int itemid)    //Take
        {
            ...
            if (requestStatus == "Accept" )
            {
                ...
                var data1 = db.Item.Select(a => new Item { ItemId = a.ItemId, ItemName = a.ItemName, ItemType = a.ItemType, Image = a.Image, Popular = a.Popular, Serial = a.Serial, UnitPrice = a.UnitPrice, DateUpdated = DateTime.Now, Quantity = a.Quantity - data.RequestQuantity }).AsQueryable();
                //.FirstOrDefault();
