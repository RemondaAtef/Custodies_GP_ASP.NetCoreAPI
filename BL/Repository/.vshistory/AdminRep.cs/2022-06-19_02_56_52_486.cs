    ...
    public class AdminRep : IAdminRep
    {
        ...
        public Item AddExistItemF(string name, int QT)   //Add Exist Item : Furniture
        {
            ...
            var data = db.Item.Where(a => a.ItemName == name && a.ItemType == "Furniture" )
                                 .Select(a => new Item { ItemId = a.ItemId, ItemName = a.ItemName, Quantity = a.Quantity + QT, UnitPrice = a.UnitPrice, 
                                     Serial = a.Serial, ItemType = a.ItemType , DateUpdated = DateTime.Now, Image = a.Image})
                                  .FirstOrDefault();
        ...
        public Request ReplyForRequest1(string userid , int itemid)   //Take
        {
            var data = db.Request.Where(a => a.TakeOrRestore == "Take" && a.UserId == userid && a.ItemId == itemid)
                                .Select(a => new Request { UserId = a.UserId, ItemId = a.ItemId, RequestID = a.RequestID, RequestQuantity = a.RequestQuantity, 
                                    RequestStatus = a.RequestStatus, StatusAfterRestore = a.StatusAfterRestore, TakeOrRestore = a.TakeOrRestore })
                                .FirstOrDefault();
        ...
        public void ReplyForRequest(string requestStatus, string userid ,int itemid)    //Take
        {
            ...
            if (requestStatus == "Accept" )
            {
                ...
                var data1 = db.Item.Where(a => a.ItemId == itemid)
                                   .Select(a => new Item { ItemId = a.ItemId, ItemName = a.ItemName, ItemType = a.ItemType, Image = a.Image, Popular = a.Popular,
                                       Serial = a.Serial, UnitPrice = a.UnitPrice, DateUpdated = DateTime.Now, Quantity = a.Quantity - data.RequestQuantity })
                                   .FirstOrDefault();
        ...
        public Request ReplayforRestore(string userid, int itemid)
        {
            var data = db.Request.Where(a => a.TakeOrRestore == "Restore" && a.UserId == userid && a.ItemId == itemid)
                                .Select(a => new Request { UserId = a.UserId, ItemId = a.ItemId, RequestID = a.RequestID, RequestQuantity = a.RequestQuantity, RequestStatus = a.RequestStatus,
                                    StatusAfterRestore = a.StatusAfterRestore, TakeOrRestore = a.TakeOrRestore })
                                .FirstOrDefault();
        ...
        public void ReplayforRestore2(string requestStatus, string userid, int itemid)
        {
          ...
          //  OwnerShip d = new OwnerShip();
            ...
            if (requestStatus == "Restore")
            {
                ...
                var data1 = db.Item.Where(a => a.ItemId == itemid)
                                     .Select(a => new Item { ItemId = a.ItemId, ItemName = a.ItemName, ItemType = a.ItemType, Image = a.Image, Popular = a.Popular, Serial = a.Serial,
                                         UnitPrice = a.UnitPrice, DateUpdated = DateTime.Now, Quantity = a.Quantity + data.RequestQuantity })
                                     .FirstOrDefault();

                db.Item.Update(data1);
                db.SaveChanges();                   // 1 - Quantity in Item Table + RequestQuantity
                ...
                db.SaveChanges();                  // 2 - Remove  Row of Ownership in Ownership TABLE    
            ...
            else if (requestStatus == "Archive")
            {
                ...
                db.Archive.Add(archive);      // 1 - Add item in Archive TABLE
                db.SaveChanges();

                if ( I.ItemId == data.ItemId &&  I.Quantity == 0)
                {
                    db.Item.Remove(I);       // 2- Remove  Row of Item in Item TABLE  ////
                    db.SaveChanges();
                }
                ...
                db.OwnerShip.Remove(ownerShip);
                db.SaveChanges();                  // 3 - Remove  Row of Ownership in Ownership TABLE 


                db.Request.Remove(data);  // 4 - Remove  Row of Request in Request TABLE
                db.SaveChanges();
