...
namespace DB3GP.BL.Repository
{
    public class AdminRep : IAdminRep
    {
        ...
        public OwnerShip UpdateDatabase(string requestStatus, string userid)
        {
            ...
            {              
                ...
                //if (d1.ItemId == data.ItemId && d1.ItemType == "Electronic Device" )
                //{
                //    d1.ItemStatus = "Owned";
                //    d1.DateUpdated = DateTime.Now;
                //    db.Item.Update(d1);
                //    db.SaveChanges();

                //}
                //else if (d1.ItemId == data.ItemId && d1.ItemType == "Furniture")
                //{
                ...
                //data.Item.ItemStatus = "owned";
                //data.Item.DateUpdated = DateTime.Now;
                // db.Item.Update(data.Item);

                //d1.ItemId = data.ItemId;
                //d1.ItemName = data.Item.ItemName;
                //d1.ItemStatus = data.Item.ItemStatus;
                //d1.ItemType = data.Item.ItemType;
        ...
        {
           var d1 =  UpdateDatabase(requestStatus , userid);

            Request d = new Request();

            var data = db.Item.Where(a => a.ItemType == "Furniture")
                                .Select(a => new Item {ItemId = a.ItemId,  ItemName = a.ItemName , ItemType = a.ItemType , ItemStatus = a.ItemStatus , Image = a.Image, Popular = a.Popular , Serial = a.Serial , UnitPrice = a.UnitPrice, DateUpdated = DateTime.Now ,Quantity = a.Quantity - d1.OwnerShipQuantity })
                                .FirstOrDefault();
